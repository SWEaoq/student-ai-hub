...
const Hero = ({ activeCategory, setActiveCategory, lang }) => {
    ...
    return (
        <section className="pt-32 pb-16 px-6 relative overflow-hidden" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <div className="max-w-5xl mx-auto text-center relative z-10">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-300 text-xs font-bold mb-6 tracking-wide uppercase">
                    <span className="w-2 h-2 rounded-full bg-purple-400 animate-ping"></span>
                    {t.hero.tag}
                </div>

                <h1 className="text-6xl md:text-8xl font-black text-white mb-6 tracking-tight leading-none">
                    {t.hero.title1} <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-500 to-red-500">{t.hero.title1_accent}</span>
                    <br />
                    {t.hero.title2} <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-500">{t.hero.title2_accent}</span>
                </h1>
