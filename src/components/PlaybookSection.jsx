import React from 'react';
import { Link } from 'react-router-dom';
import { Workflow } from 'lucide-react';
import { GUIDES, CONTENT } from '../data/content';

const PlaybookSection = ({ lang }) => {
  const t = CONTENT[lang];

  return (
    <section className="py-20 px-6 max-w-7xl mx-auto" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex items-center gap-4 mb-10">
        <div className="h-px bg-gradient-to-r from-transparent via-cyan-500 to-transparent flex-1 opacity-50" />
        <h2 className="text-3xl font-black text-white text-center uppercase tracking-widest flex items-center gap-3">
          <Workflow className="text-cyan-500" />
          {t.playbook.title} <span className="text-cyan-500">{t.playbook.title_accent}</span>
        </h2>
        <div className="h-px bg-gradient-to-r from-transparent via-cyan-500 to-transparent flex-1 opacity-50" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {GUIDES.map((guide) => {
          const content = guide.content[lang];
          return (
            <Link to={`/playbook/${guide.id}`} key={guide.id} className="block h-full">
                <div className="bg-gray-900/40 border border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-cyan-500/30 transition-all h-full flex flex-col">
                <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/10 rounded-full blur-[50px] pointer-events-none group-hover:bg-cyan-500/20 transition-all" />
                
                <div className="mb-6 flex items-center gap-3">
                    <div className="p-3 bg-cyan-500/10 rounded-lg text-cyan-400">
                    <guide.icon className="w-6 h-6" />
                    </div>
                    <h3 className="text-xl font-bold text-white leading-tight">{content.title}</h3>
                </div>
                
                <p className="text-gray-400 text-sm mb-6 border-b border-white/5 pb-4 flex-grow">
                    {content.desc}
                </p>

                <div className="text-cyan-500 text-sm font-mono flex items-center gap-2 group-hover:translate-x-2 transition-transform mt-auto">
                    {lang === 'ar' ? 'اقرأ الدليل' : 'Read Guide'} {lang === 'ar' ? <span>&larr;</span> : <span>&rarr;</span>}
                </div>
                </div>
            </Link>
          );
        })}
      </div>
    </section>
  );
};

export default PlaybookSection;
