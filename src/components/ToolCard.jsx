...
const ToolCard = ({ tool, lang }) => {
    ...
    return (
        <Link
            to={`/tool/${id}`}
            className="group relative bg-gray-900/50 border border-white/5 rounded-2xl overflow-hidden hover:border-white/20 transition-all duration-300 hover:transform hover:-translate-y-1 hover:shadow-xl block h-full"
            dir={lang === 'ar' ? 'rtl' : 'ltr'}
        >
            {/* Neon Top Border */}
            ...
            <div className="p-6">
                ...
                {/* The "Hack" Section - Reveal on Hover */}
                <div className="bg-black/40 rounded-lg p-3 border border-dashed border-white/10 group-hover:border-purple-500/30 transition-colors">
