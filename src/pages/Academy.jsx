          ...
          {categories.map((category) => {
            ...
            return (
              <AnimatedCard key={category.id} enableHover={true} className="h-full">
                ...
                >
                  <div className={`absolute top-0 right-0 w-32 h-32 sm:w-40 sm:h-40 bg-gradient-to-br ${category.color} rounded-full blur-[60px] sm:blur-[80px] opacity-20 group-hover:opacity-30 transition-opacity`} />
                  
                  <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-gradient-to-br ${category.color} opacity-20 flex items-center justify-center mb-4 sm:mb-6 text-white group-hover:scale-110 transition-transform`}>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/10">
                    <span className="text-xs sm:text-sm text-gray-500">
                      {category.stacks.length} {lang === 'ar' ? 'تقنيات متاحة' : 'stacks available'}
                    </span>
                    <span className={`text-sm font-medium bg-gradient-to-r ${category.color} bg-clip-text text-transparent group-hover:translate-x-1 transition-transform`}>
